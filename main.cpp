...
class ElementRender {
 ...
 public:
  ElementRender(ImVec2 pos, const std::string& uniqueID,
                float initialVar = 45.0f)
      : imagePos(pos),
        var(initialVar),
  ...
  void setTexture(ImTextureID texID) { textureID = texID; }
  ...
  void drawElement() {
    ...
    ImVec4 prevButtonHoveredColor =
        ImGui::GetStyle().Colors[ImGuiCol_ButtonHovered];
    ImVec4 prevButtonActiveColor =
    ...
    // bool imageButtonClicked = ImGui::ImageButton(textureID, ImVec2(150, 50),
    // ImVec2(0, 0), ImVec2(1, 1), 0, ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1));
    ...
    // Display variable
    ImVec2 variableDisplayPos =
        ImVec2(imagePos.x, imagePos.y + 55);  // Adjust position as needed
...
class MenuList {
 ...
 public:
  ...
  Component* createComponent(const string& type, ImVec2 pos,
                             const std::string& uniqueID, float initialVar) {
    if (type == "Resistor") {
...
int main() {
  ...
  vector<string> imagePaths = {
      Resistor::getImagePath(), Batt::getImagePath(), Inductor::getImagePath()
  ...
  while (window.isOpen()) {
    ...
    if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !menu.isItemPlaced()) {
      ...
      if (!selectedComponent.empty() && textureID) {
        auto component = menu.createComponent(selectedComponent, mousePos,
                                              selectedComponent, 45.0f);
        if (component) {
    ...
    //         }
    //         if (!alreadyExists) {
    //             elements.emplace_back(texID, defaultPosition, "Element_" +
    //             std::to_string(elementCounter++));
    //         }
    //     }
    // }
