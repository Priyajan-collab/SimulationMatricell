...
class Draggable {
 ...
 public:
  ...
  Draggable(int width, int height, float x, float y) : isDragging(false) {
...
class Button {
 public:
  ...
  Button(string text, Vector2f size, int charSize, Color bgColor,
         Color textColor) {
  ...
  void setBackColor(Color color) { buttonShape.setFillColor(color); }
  ...
  void setPosition(Vector2f pos) {
    ...
    float xPos =
        pos.x + (buttonShape.getSize().x - text.getLocalBounds().width) / 2;
    float yPos = pos.y +
                 (buttonShape.getSize().y - text.getLocalBounds().height) / 2 -
...
class MenuList {
 ...
 public:
  MenuList()
      : isOpen(false),
        selectedItem(-1) {}  // Default constructor, menu is initially closed
  ...
  void setTextures(const vector<Texture>& texs) {
    ...
    for (const auto& texture : textures) {
      ...
      textureIDs.push_back(reinterpret_cast<void*>(
          sprites.back().getTexture()->getNativeHandle()));
    }
  }
...
class Components {
 public:
  ...
  void setSpriteTexture(const Texture& texture) {
    ...
    Vector2f scaleFactors(rect.getSize().x / spriteBounds.width,
  ...
  void draw(RenderWindow& window) {
    // window.draw(rect); // Drawing the rectangle make the white background of
    // the rec visible
...
int main() {
  ...
  RenderWindow window(VideoMode(numCols * cellSize, numRows * cellSize),
                      "Simulation", Style::Close | Style::Resize);
  initializeGrid();
  ...
  vector<string> imagePaths = {
      "textures/ResistorIcon.png",  "textures/CapacitorIcon.png",
      "textures/InductorIcon.png",  "textures/ResistorIcon.png",
      "textures/CapacitorIcon.png", "textures/InductorIcon.png",
  ...
  Components* selectedComponent =
      nullptr;                      // Pointer to track the selected component
  ...
  while (window.isOpen()) {
    ...
    if (selectedItemIndex != -1) {
      if (selectedItemIndex !=
          lastSelectedItemIndex) {  // creates a new element if a new item is
                                    // selected

        Components component;
        component.init(
            Vector2f(100.f, 50.f),
            Vector2f(300.f, 150.f));  // sets the intial size and postion

        component.setSpriteTexture(textures[selectedItemIndex]);
        components.push_back(component);
        selectedComponent =
            &components.back();  // Set as the  new selected component
        lastSelectedItemIndex =
            selectedItemIndex;       // Update the last selected index
